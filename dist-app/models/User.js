...
var createUser = function createUser(name, hashedPassword) {
  _DatabaseService.default.one('INSERT INTO public.user(name, hashed_password) VALUES($1, $2) RETURNING id', [name, hashedPassword]).then(function (data) {
...
var updateUser = function updateUser(id, name, money) {
  _DatabaseService.default.one("UPDATE public.user\n          SET name = $(name),\n              money = $(money)\n          WHERE id = $(id)\n          RETURNING name, id, money", {
    name: name,
    id: id,
    money: money
  }).then(function (data) {
